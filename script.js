...
function startMagicDust() {
  if (prefersReducedMotion || hasTouchPointer) return;
  setInterval(() => {
    if (!document.hidden) createDust();
  }, 700);
}
runWhenIdle(startMagicDust);
...
function initCustomCursor() {
  if (hasTouchPointer || prefersReducedMotion) return;
...
runWhenIdle(initTestimonialsSlider);
